...
Everything except the optional custom server runs client-side, so the built `dist/` folder can be served statically.

1. Set a relative base so project pages resolve assets correctly:
...
2. Publish `dist/` with `actions/upload-pages-artifact` + `actions/deploy-pages` (or push to a `gh-pages` branch), and add `public/.nojekyll`.
3. Pages serves over HTTPS, which satisfies the secure-context requirements for `getDisplayMedia`, the microphone, WebGPU, WASM, workers, IndexedDB and `crypto.randomUUID`.

Two limitations to expect on Pages:

- Pages cannot send `COOP`/`COEP` headers, so onnxruntime's threaded WASM build likely falls back to single-threaded. The WebGPU path is unaffected.
- The Whisper model is downloaded from the HuggingFace CDN on first use (~75 MB for `tiny.en`, more for larger models). That works from Pages but can be blocked on locked-down networks.
